...
session-to-pane correlation is the (future) runner's job: it calls `pane.report_agent` with
`source: "interlock"`, `agent: "<graph>/<node>/<session>"` when it starts a pane, and the face
resolves "which pane is session X" by querying herdr's own `agent.list`/`agent.get` filtered on
that source, rather than teaching the ledger about panes. The write direction stops there: the
...
*Revisit:* if herdr's own docs, or a live check against a running instance, show a second
`source` on `pane.report_agent` doesn't compose cleanly with herdr's native detection in the same
sidebar row — verify before depending on it. Writing interlock's lifecycle back into herdr's
...
*Nothing has bent yet — this note has not met real use.*
