...
| 2026-09-11 | `node cancel`, `node reset` and `gate waive` are built, dispatching through the same subprocess boundary D15 names; `pause` is not, confirming D15's own flagged gap on real code rather than in prose alone | D15 | `Outcome`'s six kinds (`cleared`, `held`, `reset`, `failed`, `cancelled`, `superseded`) have no kind naming a voluntary, non-failure hold — `held` requires a `failure` string a pause never has. Adding a seventh kind is a union-arm shape change (`isOutcome`'s switch, every exhaustive `Outcome.kind` switch in this package and `ledger`), not a verb session's call to make alone. | Yes, nothing pause needs is removed; it waits on a kind that does not exist yet | A seventh `Outcome` kind, with its own event version, is the only truthful entry point for `pause` — not a stretch of `held`, and not a workaround in the face or the CLI. |
| 2026-09-11 | Navigation — the arrows, `j`/`k`, `?` — now renders from the world already in hand instead of a fresh join; a fresh read still happens on Enter, Escape, `r`, the two-second timer and after a verb dispatch | D14 | The first person to open the face live, in a real herdr pane, saw Enter, Escape and `?` do nothing for seconds at a node carrying nine attempts from the day before, none ever swept. Liveness was read off the projection's `leaseExpired` flag alone, a fact only a sweeper's own event sets; every one of those nine still read as live, and a fresh join on every key called herdr's agent status once per attempt per keypress — the same join the node level ran on `?`, which touches no data at all. | Yes, `refreshWorld` is one call away on the keys that still take it; nothing about a fresh read was removed, only when it fires | Two separate things were wearing one name. Liveness is `lease.expiry` against the clock, `leaseExpired` a second, independent way to lose it — that part is a correctness fix, not a bend, now living in the projection itself. What actually bends D14 is narrower: a render that changes nothing about which graph, node or session is in view has nothing to read fresh. |
