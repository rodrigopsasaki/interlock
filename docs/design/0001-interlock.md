...
| 2026-09-12 | `packages/*/src` carried the no-comments convention as a sentence in `AGENTS.md` and nothing else: 108 comment lines had leaked across debrief, face, ledger, runner and verifier by the time anyone counted, every one of them a real why a reader would otherwise miss, dropped only because no session before this one checked | Conventions | A convention with no gate is exactly as strong as whoever remembers to grep for it before merging, and a hundred-plus leaked lines across five packages over many sessions is what that looks like in practice | Yes, every dropped comment's reason is carried into the commit body that removed it, file by file, so nothing the comments knew is gone, only relocated to where the convention already said it belongs | The standing table gains a `comments` gate running `scripts/noComments.ts`, and the check has to walk the real parse tree, not an independent scanner: a scanner run with trivia left on correctly ignores `//` inside a string or template literal but not inside a regex literal, because telling a `/` from a comment opener needs the same expression-position context the parser already tracks -- caught on this repository's own `/^https?:\/\//` in `substrate/src/address.ts`, which a first pass of the gate misread as opening a comment. Walking the source file's own children and collecting both leading and trailing comment ranges at every token fixes it, and is the only shape of check that could have caught what a naive pattern match already couldn't: a URL or a pattern carrying `//` is exactly the false positive the rule must not produce. Three of the dropped comments, in debrief's, face's, ledger's and runner's own `validate.ts`, said out loud that `prop`, `isRecord` and `isString` are hand-mirrored across four packages; collapsing them to one shared module is a follow-up, not done here |
| 2026-09-12 | `interlock plan` forked `run.ts`'s whole pane orchestration by copy — lease, worktree, brief, setup, pane, prompt, wait, judgement, receipts, pane close — because the session that built it judged touching `run.ts` too risky to parameterize with no second caller yet proving what the parameterization needed to look like; its own review, three read-only passes agreeing the node's acceptance was met, still flagged the copy as the one thing worth an hour before merge. The same review found the gate-judging underneath it ran the standing gates and could record a node cleared before plan.ts had so much as looked at what the session actually produced, so a malformed graph, or one whose own `id` didn't match the graph being planned, could clear anyway — cleared is a terminal outcome once `ledger.append` records it, so a check added after judging is a check added too late to matter | D5, D13, axiom 4 (compose, don't reinvent) | Two verbs shared one orchestration by copy until the second was reviewed, which is exactly the point a guess about the shared shape stops being a guess: `run.ts`'s own 400-odd lines and `plan.ts`'s copy of them read close enough by then that the seam was obvious, not invented. And a receipt proving the standing gates passed was never a receipt proving the acceptance itself — "produce a valid graph" — was met; the two had been quietly treated as the same proof | Yes, every `run.test.ts` (33) and pre-existing `plan.test.ts` case passes unchanged, narration included; the extraction changes no behavior, and the produced-graph check moved earlier in the sequence, not added new | `driveInteractiveSession` (`packages/runner/src/sessionDrive.ts`) is the one pane-orchestration sequence now, taking a role, a brief-composing closure, and two further seams — a pre-worktree check, a pre-judgement check — as its only parameters; `run.ts` drops to 177 lines and supplies neither optional seam, `plan.ts` drops to 294 and supplies both. A session's own produced-artifact check runs through the pre-judgement seam, before `judgeWorktree` ever appends an outcome, and records held with the refusal as its reason on any refusal, itself, rather than trusting the standing gates' own outcome to also mean the deliverable was sound |
| 2026-09-12 | `0003-translator`'s `first-address` node declared its `witnessed` gate `kind: human`, the first human gate on a real node outside the graph-level `approved` gate D20 already built. Judgement never checked a gate's declared kind: with no `run` it spawned an empty command anyway, recorded a receipt over a non-zero exit, and moved the gate to `blocked`, narrating `witnessed: exit -1`. The only verb a person had over a gate was `waive`, and waiving a criterion is giving up on it, not meeting it — D21 already said a human criterion is met by a named person clearing it with a because, and no verb did that | D21, vocabulary | `gateJudge` trusted `commandFor` to carry a `run` for every declared gate and never asked what kind a gate was before spawning one; a human gate has no command to run by design, not a misconfigured one, and the loop could not tell the two apart | Yes, the pre-fix `blocked` receipts and their narration stay in the journal exactly as written; nothing is retracted, only judged differently from here on | `GateCommand` (`packages/runner/src/gateCommand.ts`) now carries a gate's declared `kind` alongside its `run`, sourced from the same `GateDeclaration`/`StandingGate` entries `commandFor` already read, for standing and node gates alike, though the standing-gate schema still requires `run` on every entry, so only a node's own gates are human in practice today. `gateJudge` reads that `kind` before spawning anything: a pending human gate is never run, narrates `<gate>: awaits a person`, and holds the node naming the gate and that a person's verb clears it, distinct from the generic "gate(s) not satisfied" sentence a failed command leaves; a satisfied or waived human gate clears like any other. `interlock gate clear <graph> <node> <gate> --by <who> --because <why>` is that verb, beside `waive`: it refuses an unknown graph, node or gate, a gate that is not `kind: human`, a missing `--by` or `--because`, and a gate already `satisfied` or `waived` with the same terminal-rule sentence `waive` uses. It follows `graph approve`'s shape more than `waive`'s own — it mints a fresh human receipt over the node's scope at the repository's HEAD rather than reusing one, because a human criterion that was never run has no prior receipt to point at. `schemas/parts/gate-declaration.json` had already documented "a human gate carries no run" before this session touched it; the schema was ahead of the loader that was supposed to honor it, so no schema change was needed, only the code catching up. `session show` now shows a human-cleared gate's because behind the same `--because` flag a decision's because already sits behind; `interlock evidence`'s existing human-derivation-satisfied mapping to a professed item, built for `approved`, is generic in the gate id and needed only a second test to confirm it reads `witnessed` the same way. Left standing: the face's accountable verbs (`packages/face/src/verb.ts`) stay `waive`-only; offering `clear` beside it needs `PositionGate` to carry a gate's declared kind, which `position@v1` does not, and threading that through position.ts, its schema and several face and cli test fixtures is a larger change than this fix — a follow-up, not done here |
...
| 2026-09-15 | A corrected current `debrief@v2` reaches the receiver while its preceding bytes remain under a session's `revisions/` directory, and one corpus rule had treated every file as a current artifact | I8, compatibility | Current validity and retained history answer different questions: the schema must reject an invalid current file, but its exact prior bytes are evidence that must remain available for repair and drilldown | Yes, the schema reader still rejects the retained invalid payload directly; only a regular `sessions/<graph>/<node>/revisions/<sha256>.yaml` file whose raw bytes match its lowercase SHA-256 filename is retained history, and revise keeps the old bytes before installing a compatible candidate | `debrief validate` composes the published schema only at its v2 door, while legacy readers and revise continue to read old bytes. The real-file corpus proves custody affirmatively for the one hash-addressed history location; moved, renamed, tampered, malformed-name, and symbolic-link files remain ordinary or rejected rather than escaping validation. |
| 2026-09-16 | Generated reference examples competed by file size across every live artifact, so a newly completed valid session could replace a published example without any change to the reference's declared sources | I8 | The page retained a rendered source path but no deliberate selection; source arrival and byte size were accidental authority over a published compression | Yes, the existing chosen artifacts remain the selections, their paths are rendered beside their contents, and shapes with no selection still render the no-example result | The reference now names the retained source path and expected shape for each represented example, validates that exact candidate through the existing schema registry, and refuses a missing, wrong-shape, or invalid selection. The independent full-artifact corpus check remains responsible for every live artifact, including unselected sessions. |
| 2026-09-16 | An authored `debrief@v2` carried a nonexistent session-start SHA, and ordinary revision correctly treated it as immutable even though a candidate held the real start | D8, I8 | A correction needs to retain the mistaken debrief without allowing an agent or a caller-supplied SHA to reassign a session | Yes, ordinary revision remains unchanged; recovery is explicit, read-only until the existing exact-byte retention and atomic selection run, and every refusal leaves authored files unchanged | The CLI may supply recovery proof only when the current start is not a commit, one complete typed start record matches the immutable worktree brief and graph-base acceptance, and Git finds one ancestry-path introduction of that session identity whose original bytes still equal the local brief. The candidate must name that exact commit and a real descendant head; missing, duplicate, foreign, mismatched, malformed, or torn evidence refuses. |
| 2026-09-24 | `local.yaml`'s `runtime:` block was where a model was named, once per repository; a per-machine catalogue and a per-attempt `--runtime` replace it, and the session records the choice | local shape, D1, D2, D10, D13, “Strategy is declared; model is measured” | The door wants to say which model runs which node, and the record must say which did. | Yes, the `runtime:` block still reads as the runtime named `default`; pre-v6 sessions read as `unknown`, never invented | A model choice belongs to the attempt and its session record, while the catalogue only says which runtimes this machine can ask the door to start. |
| 2026-09-24 | The first Kimi runtime start (`k3-first-run`): herdr read kimi as idle under its workspace-trust prompt, so the configured startup answer, sent only on a `blocked` reading, never fired; the opening prompt's own Enter selected the preselected "Trust this folder" thirteen milliseconds after the send, the prompt text was lost, and the twenty-second prompt-taken wait refused an agent that was genuinely idle again; the refusal closed the pane and abandoned the lease, and a person split a new pane, started the agent twice (the first manual start exited within a minute) and re-sent the prompt by hand | The runner's startup answers and refusal path, D26 | Startup answers key on herdr's `blocked` status, and a dialog the runtime reads as `idle` is invisible to them; the Codex hooks modal lost the prompt the same way, but there a person dismissed it while the runner still held the pane, and here the prompt's own keystroke dismissed the dialog and the refusal closed the pane before anyone could recover it in place | Yes, the abandoned run's narration and lease stay in the journal, the trust record persists in the agent's own workspace-trust store, and herdr's server log keeps both panes' history; the recovered session works on from the same brief and session record | A sent prompt is not a delivered one: its Enter can answer a dialog instead of submitting the text, and prompt-taken evidence on this runtime errs both ways — the runner's wait refused while the agent was genuinely idle at startup, and herdr's own CLI reported the person's re-sent prompt as an error although the agent took it within half a second. A trust prompt that reads as `idle` needs detection, not a startup answer, because the answer only exists for a runtime herdr can see is blocked. |
