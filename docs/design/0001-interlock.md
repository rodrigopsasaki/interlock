...
| | |
| --- | --- |
| Status | Direction decided by Rodrigo. Shape drafted in conversation. Nothing built. Name decided. TypeScript on Node. |
...
| 2026-09-09 | Rebasing the scaffold branch onto main rewrote every commit SHA; the debrief's `head_sha` and any receipt addressed by commit SHA went stale although not one byte of the tree changed | Receipts, D5 | Content-addressing by commit SHA conflates content with history. | Yes, the pre-rebase SHA is in the debrief and the trees are provably equal | Address receipts by tree hash, not commit SHA: a rebase that changes no bytes keeps its receipts; a commit that changes bytes invalidates them. The commit SHA stays on the receipt as history, not identity. |
