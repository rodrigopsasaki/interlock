...
| 2026-09-11 | A live run's agent stopped for a shell approval, the wait narrated the pane and held as D26 says, and the person answered no. In the agent's own runtime a denied command ends its turn outright, with no `working` in between; herdr surfaced `done`, and the wait, having already learned to sit through `blocked`, read that `done` arriving straight off a person's turn as the session's end and judged on the spot, holding the node on a missing debrief while the person kept typing into a pane the wait had already left | D26 | `blocked` names a person's turn, not a promise that the turn ends in `working`; a runtime that refuses a command hands control back without ever reporting it took the wheel. Judging the instant a settled status follows one on a person's turn treats "hasn't resumed yet" as "isn't going to," which is a guess the wait has no standing to make | Yes, the pre-fix run's judgement and receipts are unchanged; nothing is retracted, only how a settled status right after a person's turn is read from here on | Once the wait has seen `blocked`, a settled status no longer ends it outright: it opens a bounded window watching for `working`, re-armed in full on every settle a person's turn produces, and gives up only when the window closes with no resume or the run's own deadline arrives first, whichever comes first. The window is a per-machine duration, defaulted rather than mandated, since most sessions never draw on it |
