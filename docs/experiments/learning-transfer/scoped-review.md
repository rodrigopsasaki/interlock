...
That is a source-checked match for the stated boundary: selection can narrow the substrate request, but it does not replace the runner's authority. The selector's authority is nevertheless the complete current tracked set, not the repository brief's prior `scope` list; this is intentional in the observed flow because that list is overwritten from `gitTrackedFiles` after the check. This review makes no claim about paths outside the runner's exact-string and tracked-file checks, such as how another component may later resolve a selected path.
