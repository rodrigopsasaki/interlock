...
export function parseDiscovery(raw: unknown, index: number): Result<Discovery, string> {
  ...
  if (!isString(id)) return err(`discovery ${index}: "id" is missing or not a string`);

  const what = prop(raw, "what");
  if (!isString(what)) return err(`discovery "${id}": "what" is missing or not a string`);

  const foundAt = prop(raw, "found_at");
  if (!isString(foundAt)) return err(`discovery "${id}": "found_at" is missing or not a string`);
