...
export function parseDecision(
  ...
  if (!isString(id))
    return err(`decision ${index}: "id" is missing or not a string`);

  const what = prop(raw, "what");
  if (!isString(what))
    return err(`decision "${id}": "what" is missing or not a string`);

  const because = prop(raw, "because");
  if (!isString(because)) return err(`decision "${id}": missing "because"`);

  const restsOn = prop(raw, "rests_on");
  if (!isStringArray(restsOn))
    return err(`decision "${id}": "rests_on" must be a list of strings`);

  const hunks = prop(raw, "hunks");
  if (!isStringArray(hunks))
    return err(`decision "${id}": "hunks" must be a list of strings`);
