...
function renderBody(state: FaceState, world: FaceWorld, sessionText: string | undefined): string {
  switch (state.level) {
    case "plans":
    ...
    case "node": {
      const node = world.position?.nodes.find((candidate) => candidate.id === state.selection.node);
      return node === undefined ? "(loading node…)" : renderNodeFrame(node, state.index);
    }
    case "session":
...
function renderBottomLine(state: FaceState): string {
  if (state.kind === "prompting") {
    ...
    const value = state.prompt.field === "because" ? state.prompt.because : state.prompt.by;
...
export function renderScreen(
  ...
  const withHelp =
    state.kind === "browsing" && state.help ? `${body}\n\n${renderHelpOverlay()}` : body;
