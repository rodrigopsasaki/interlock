...
import { runDebriefPreview } from "./debrief/preview.ts";
...
export async function run(argv: readonly string[]): Promise<CommandResult> {
  ...
  if (group === "debrief" && action === "preview") {
    return runDebriefPreview(argv.slice(2));
  }
  ...
  if (group === "debrief" && action === "prepare") {
    return runDebriefPrepare(argv.slice(2));
  }

  if (group === "debrief" && action === "file-derived") {
    return runDebriefFileDerived(argv.slice(2));
  }
  ...
  if (group === "evidence") {
    return runInterlockEvidence(argv.slice(1));
  }
