...
describe("interlock run", () => {
  ...
  it("retries through every attempt, then keeps the pane open and judges as interrupted when the opening prompt is never taken", async () => {
    const cwd = fixture({ graphYaml: graphYamlWithExpectOutput });
    await runGraphApprove(["demo", "--by", "Rodrigo Sasaki", "--because", "looks right"], { cwd });
    ...
    expect(lines).toContain("prompt not taken after 20000ms; retrying (2 of 2): opening prompt");
    ...
    const runtime: Runtime = {
      ...
      waitUntil: (agent, until, timeoutMs) =>
        until.length === 2
          ...
          : stubRuntime().waitUntil(agent, until, timeoutMs),
      read: () => Promise.resolve(ok(blocked ? "trust this project? [Down/Enter]" : "")),
