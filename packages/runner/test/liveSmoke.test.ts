...
import { createHerdrRuntime, defaultHerdrSocketPath } from "../src/herdr/adapter.ts";
...
describe("live smoke", () => {
  it("connects to the real herdr socket if one is present, or skips honestly", async () => {
    ...
    if (!existsSync(socketPath)) {
      console.log(`live smoke: skipped, no herdr socket at ${socketPath} (herdr was not started).`);
      return;
    }
