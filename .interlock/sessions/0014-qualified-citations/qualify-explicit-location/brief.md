...
gates:
  ...
  - id: citation-proof
    kind: command
    run: pnpm --filter verifier --fail-if-no-match exec vitest run
      test/foundAt.test.ts test/verify.test.ts --reporter=verbose
    expect_output: Tests +[1-9][0-9]* passed
  - id: reuse-filter-proof
    kind: command
    run: pnpm --filter runner --fail-if-no-match exec vitest run
      test/gateJudge.test.ts --reporter=verbose
    expect_output: Tests +[1-9][0-9]* passed
scope:
  - .gitignore
