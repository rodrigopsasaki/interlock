---
interlock: brief@v1
graph: 0040-explicit-citation-boundary
node: enforce-explicit-citation-boundary
role: worker
gates:
  - id: typecheck
    kind: command
  ...
  - id: explicit-citation-parser-proof
    kind: command
    run: pnpm --filter verifier exec vitest run test/foundAt.test.ts
    expect_output: Tests +[1-9][0-9]* passed
  - id: explicit-citation-translation-proof
    kind: command
    run: pnpm --filter runner exec vitest run test/debriefAuthoringGuidance.test.ts
    expect_output: Tests +[1-9][0-9]* passed
scope:
  - .gitignore
...
graph_base_sha: 84692ec00443757cc9a5644bb94c1b7619395b3c
session: a36e13dd-0ef8-4b58-8095-638668a652fb
---

## Task

Deliver the complete approved citation-boundary repair with its actual verifier
and translation proof. Read AGENTS, graph acceptance and current sources. Received
hypotheses are provisional: check, reject or leave unused; do not claim their
presence caused a better result. Keep both valid citation forms and refusal
discriminators. Preserve earlier artifacts and all unrelated work.

Actual authorship is codex/gpt-5.6-terra. Use date -u for actual note times,
full Git IDs and mise exec. Retain yielded exec handles; one local check at a
time. Use the declared focused commands and lint before handoff; the harness
runs standing gates independently. Use ordinary derived first filing after the
source commit; retain candidates and stop after committed canonical debrief.
One25-minute implementation attempt. No agents, dependencies, trust/permission/
config changes, external inference, live-brain calls, push, merge or deployment.
Per-command unsigned commits are allowed if signing is unavailable.
...
- [hypothesis, path packages/verifier/test/foundAt.test.ts] Added final-real-line fixture proof for both newline states, ordinary prose punctuation, and the adjacent out-of-bounds lines. (because The parser already removes only the newline-created terminal empty line; the repair needed discriminating assertions rather than a production change.)
...
- [hypothesis, repository] Separated the supplied translation, receiver response report, and textual Context slice comparison in the absorb narration. (because A valid v2 gate-judge route always derives observed translation data, including an empty payload, while the public narrator alone can report an unavailable caller observation; keeping the request object separate preserves legacy omission and the durable outbox bytes.)
  derivation: agent:codex:gpt-5.6-terra

- [hypothesis, path packages/verifier/src/foundAt.ts] Planned one approval-gated implementation node that routes bounded explicit nonquoted filename-and-range claims through the existing checkFoundAt range, path, and quotation branch without changing ordinary fallback citations. (because The current parser seam owns both the extension heuristic and the downstream checks, so one bounded change can correct evidence.txt and extensionless basenames while preserving command and genuine out-of-band behavior.)
  derivation: agent:codex:gpt-5.6-terra

- [hypothesis, path packages/verifier/src/foundAt.ts] Required a new unrecognized-extension or extensionless locator to occupy the first nonblank citation token, raw or locator-wrapped in backticks, with a separate quotation; later ordinary-prose version:2 is not a file claim. (because This preserves established slash and known-extension prose forms while making standalone Makefile:2 useful and preventing the extension repair from assigning arbitrary prose a new citation meaning.)
  derivation: agent:codex:gpt-5.6-terra

- [hypothesis, repository] Required direct parser discriminators together with a real verifyDebrief-to-evidenceOf assertion for a rooted evidence.txt discovery and its unrooted quote variants. (because A direct parser mark does not show whether the repaired discovery retains its resolved path through evidence translation, where an out-of-band root would instead become repository-scoped.)
  derivation: agent:codex:gpt-5.6-terra

- [hypothesis, path packages/verifier/src/foundAt.ts] The explicit-location parser drops a path-and-suffix match unless its path passes pathLike before the explicit range, path, and quotation checks run. (because A bare filename with an unrecognized extension can reach the quoted out-of-band fallback instead of being checked as an explicit file location.)
  derivation: agent:codex:gpt-5.6-terra

### Absence

- [observed, path packages/runner/test/debriefAuthoringGuidance.test.ts] Nothing is held about packages/runner/test/debriefAuthoringGuidance.test.ts.
