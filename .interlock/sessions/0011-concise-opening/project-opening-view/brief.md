...
gates:
  ...
  - id: opening-view-proof
    ...
    run: pnpm --filter runner --fail-if-no-match exec vitest run
      test/openingPrompt.test.ts test/sessionBrief.test.ts --reporter=verbose
    expect_output: Tests +[1-9][0-9]* passed
  - id: runtime-opening-proof
    kind: command
    run: pnpm --filter cli --fail-if-no-match exec vitest run test/run.test.ts
      --reporter=verbose
    expect_output: Tests +[1-9][0-9]* passed
scope:
  - .gitignore
...
session: 2209430a-dc07-42fe-b99a-f5308990ce5c
---
