...
gates:
  ...
  - id: review-artifact
    kind: command
    run: test -s docs/experiments/learning-transfer/scoped-review.md
scope:
  - .gitignore
...
session: 8c9bb08e-7418-479f-aefa-5116d5bc4e1f
---
...
- [hypothesis, repository] Added optional context_scope to brief@v1 with nonempty, unique structural schema checks and reader-side path checks that preserve omitted-selector legacy behavior. (because The selector is advisory retrieval input, so its structural schema must not claim semantic path safety that the existing normalize-based reader owns.)
  derivation: agent:codex:gpt-5.6-terra

- [hypothesis, repository] Kept full gitTrackedFiles scope canonical and in the ledger, while dispatching context with a validated selector only when every selected spelling exactly matches a tracked path. (because A reader-safe alias or missing path must produce a typed refusal before context rather than normalize into a different authority or make an unbounded request.)
  derivation: agent:codex:gpt-5.6-terra

- [hypothesis, repository] Preserved context_scope entries in the derived opening metadata and proved the actual addressed 3,000-path runtime query carries exactly its three selected paths before prompt. (because Sessions need to see the small selector they receive, while the full inventory remains recoverable only through the canonical brief and its existing count/SHA metadata.)
  derivation: agent:codex:gpt-5.6-terra
